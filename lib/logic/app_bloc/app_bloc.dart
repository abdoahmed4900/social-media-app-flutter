...
import '../../view/screens/show_post_screen.dart';
...
class AppBloc extends Bloc<AppEvent, AppState> {
  ...
  Future removePost({required String postId}) async {
    add(RemovePost());
  ...
  void addComment(postId, String comment, context) async {
    final postData = await postsCollection.doc(postId).get();
    ...
    final receiverData = await usersCollection.doc(post.userId).get();

    UserModel receiverModel = UserModel.fromMap(receiverData.data()!);

    postsCollection.doc(postId).update(post.toMap()).then((value) {
      add(AddComment());
      if (receiverModel.uId != userModel!.uId) {
        NotificationsHelper.sendNotification(
          notificationMessage: '${userModel!.name} commented on your post',
          receiverNotificationToken: receiverModel.notiifcationToken,
        );
        notificationType = NotificationType.comment;
        goToOnePost(context, post);
      }
    });
  }

  void goToOnePost(context, postModel) {
    push(ShowPost(postModel: postModel), context);
  }
  ...
  Future sendPhoto({required String receiverId, required String source}) async {
    final otherUserData = await usersCollection.doc(receiverId).get();
    ...
    final receiverDoc = await usersCollection
        .doc(receiverId)
        .collection('chats')
        .doc(uId)
        .collection('messages')
    ...
    final senderDoc = await usersCollection
        .doc(uId)
        .collection('chats')
        .doc(receiverId)
        .collection('messages')
    ...
    for (int i = 0; i < pickedPhotos.length; i++) {
      add(AppLoad());
      storageRef
          .child('images/messages/${senderDoc.id}/$i')
